...
pub fn load_workspace_manifest(workspace_path: &Path) -> Result<WorkspaceManifest, String> {
    ...
    if !manifest_path.exists() {
        return Err(format!("Workspace manifest not found: {}", manifest_path.display()));
    }
...
pub fn load_all_charts(base_dir: &Path, manifest: &WorkspaceManifest) -> Result<Vec<ChartInstance>, String> {
...
pub fn chart_to_summary(chart: &ChartInstance) -> ChartSummary {
    let date_time = chart.subject.event_time
